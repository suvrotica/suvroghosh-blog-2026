...
Here $L$ is **lower triangular** (zeros above the diagonal) and $U$ is **upper triangular** (zeros below the diagonal). [cite_start]The $P$ is a permutation matrix—a bookkeeping gadget that means “we swapped some rows to keep the arithmetic sane.” [cite: 5]
...
[cite_start]That second phase is much cheaper than re-eliminating from scratch. [cite: 5]
...
Imagine you’re trying to solve a system of equations. [cite_start]Gaussian elimination’s idea is simple: use the first equation to knock out the first variable from all equations below it, then move to the next variable, and so on until the system becomes a staircase. [cite: 5]
...
[cite_start]So LU is not a different magic trick—it’s the same trick, but written as a clean factorization. [cite: 5]
...
* **Ancient China, ~150 BC onward:** Methods equivalent to elimination appear in *The Nine Chapters on the Mathematical Art* (later referenced by title around 179 AD), with commentary by Liu Hui in the 3rd century. [cite_start]That’s elimination as a computational craft, centuries before European textbooks got around to it. [cite: 5]
* [cite_start]**Cambridge, 1669–1670 → published 1707:** Isaac Newton wrote notes laying out how to solve simultaneous equations; they were later published as *Arithmetica Universalis* (1707). [cite: 5]
* [cite_start]**Early 1800s, Germany:** Carl Friedrich Gauss refined elimination notation for the kinds of linear systems that show up in least-squares and surveying; a key reference point is Gauss (1810) in the historical record. [cite: 5]
* [cite_start]**United States Coast Survey, 1881:** Myrick Hascall Doolittle (a “human computer,” in the old job-title sense) pushed elimination into an efficient tabular form that working agencies could actually run at scale. [cite: 5]
* [cite_start]**Kraków / Jagiellonian orbit, 1938:** The Polish astronomer Tadeusz Banachiewicz is credited with introducing LU decomposition explicitly as matrix factorization (the modern conceptual framing). [cite: 5]
* [cite_start]**MIT, 1941:** Prescott Durand Crout reorganized elimination so it fit the strengths of mechanical calculating machines—an early example of algorithms evolving to match the hardware of their era. [cite: 5]

[cite_start]A fun historical footnote: the term “Gaussian elimination” itself is a comparatively late naming habit—Wikipedia notes it was only called that widely in the 1950s, partly via historical confusion. [cite: 5]
...
LU is one of the silent pistons inside modern computation. Any time you see “solve a linear system,” you’re in LU territory (or a close cousin like QR or Cholesky, chosen for special structure). [cite_start]LU is the default general-purpose engine because it’s fast, reusable, and conceptually tied directly to elimination. [cite: 5]
