...
<div class="card my-8 border-l-4 border-l-gold bg-white dark:bg-neutral-800 p-6 shadow-xl">
	<div class="mb-4 flex flex-col gap-4 sm:flex-row sm:items-end">
		...
		<div class="flex gap-2 text-xs">
			<button 
                onclick={() => expression = 'z^2 + 1'}
                class="rounded bg-neutral-200 px-2 py-2 font-mono text-neutral-700 hover:bg-gold hover:text-white dark:bg-neutral-700 dark:text-neutral-300 dark:hover:bg-gold cursor-pointer transition-colors"
            >
                z²+1
            </button>
			<button 
                onclick={() => expression = '1/z'}
                class="rounded bg-neutral-200 px-2 py-2 font-mono text-neutral-700 hover:bg-gold hover:text-white dark:bg-neutral-700 dark:text-neutral-300 dark:hover:bg-gold cursor-pointer transition-colors"
            >
                1/z
            </button>
			<button 
                onclick={() => expression = 'sin(z)'}
                class="rounded bg-neutral-200 px-2 py-2 font-mono text-neutral-700 hover:bg-gold hover:text-white dark:bg-neutral-700 dark:text-neutral-300 dark:hover:bg-gold cursor-pointer transition-colors"
            >
                sin(z)
            </button>
